...
| **R8** replica id in output | **Nothing.** | **0 d** | Corrected 2026-09-10. An earlier version of this row carried 0.5 d for "add `replica_id` and `arrived_at` columns". Run 4b's output was then re-checked: `canary`'s `request_metrics.csv` already emits `replica` (col 25), `request_arrived_at` (col 24) **and** `request_num_prefill_tokens_cached` (col 20). The 0.5 d applied to `main`, which we are not using. |
...
| | **Total** | **13–22 d** | 3–5 + 3–5 + 1–2 + 4–7 + 0 + 2–3 |

**What the 13–22 d total does NOT include** — stated explicitly so the number is
not read as the cost of the whole project:

| Excluded work | Milestone | Status |
|---|---|---|
| Implementing the policies themselves — B1, B2-req, **B2-tok**, B3, B4 | M5–M8 | additional, not estimated here |
| The symmetric tuning protocol for baseline and treatment parameters | M5/M8 | additional, not estimated here |
| Resolving Mooncake's context-length constraint (falsifier **F6**) — long-context model config *with matching profiling data*, or a documented filtering/scaling policy | M2/M4 | additional, **not yet estimated**; a research decision, not a coding task |
| Analysis harness: bootstrap CIs, paired comparisons, effect sizes | M5 | additional |
| E8 fidelity comparison against real vLLM | M11 | additional |

The 13–22 d figure is **only** the cost of closing the R1–R8 gaps in the
simulator base.
